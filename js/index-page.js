...
function updatePriceUi() {
  ...
  if (isRtl) {
    rangeProgress.style.right = `${Math.max(0, minPercent)}%`;
    rangeProgress.style.left = `${Math.max(0, 100 - maxPercent)}%`;
  } else {
...
function renderProducts(list) {
  ...
  if (!list.length) {
    if (stateBox) {
      ...
      stateBox.textContent = i18n[lang].noProducts;
    }
    return;
  }
  if (stateBox) {
  ...
  list.forEach((product, index) => {
    ...
    card.innerHTML = `
      ...
      <div class="product-info">
        ...
        ${product.tag ? `<div class="tag-under-name">${escapeHtml(product.tag)}</div>` : ""}
